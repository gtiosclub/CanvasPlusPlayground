...
// Adapted from: https://github.com/dejager/wallpaper/blob/main/Wallpaper/GrainGradient.metal
...
half4 grainGradient(float2 position, float4 bounds, float size, float time, device const half4 *colors, int count) {
    ...
    half4 result = gridInterpolation(coords, colors, float4(gridRange), gridSize, gridLastIndex, time * 0.30);
